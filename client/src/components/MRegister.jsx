...
const MRegister = () => {
    ...
    const handleSubmit = async (e) => {
      e.preventDefault();
      try {
        let res = await axios.post("http://localhost:3001/mRegister", inputs);
      ...
      <FormControl>
        ...
        <FormControl>
          ...
          <Input  placeholder='Enter password' type='password' name='password'onChange={handleChange} />
            ...
            <Link to="/manufacturer/login"><Button  variant="contained" onClick={handleSubmit} style={{margin:"1rem"}}>Register</Button></Link>
            <p> Already have an account? <Link to="/manufacturer/login" style={{ textDecoration: "none" }}>
